...
class BikeSaleController extends Controller
{
    ...
    public function update(Request $request,$id)
    {
        ...
        if(isset($doc_nid)){
            ...
            unlink(public_path('uploads/bike-sales/'.$bikeSale->doc_nid));
        }
        if(isset($doc_reg_card)){
            $sale_data['doc_reg_card'] = $this->documentUpload($doc_reg_card);
            unlink(public_path('uploads/bike-sales/'.$bikeSale->doc_reg_card));
        }
        if(isset($doc_image)){
            $sale_data['doc_image'] = $this->documentUpload($doc_image);
            unlink(public_path('uploads/bike-sales/'.$bikeSale->doc_image));
        }
        if(isset($doc_deed)){
            $sale_data['doc_deed'] = $this->documentUpload($doc_deed);
            unlink(public_path('uploads/bike-sales/'.$bikeSale->doc_deed));
        }
    ...
    {
        $bikeSale = BikeSale::find($id);
        if($bikeSale->doc_nid) unlink(public_path('uploads/bike-sales/'.$bikeSale->doc_nid));
        if($bikeSale->doc_reg_card) unlink(public_path('uploads/bike-sales/'.$bikeSale->doc_reg_card));
        if($bikeSale->doc_image) unlink(public_path('uploads/bike-sales/'.$bikeSale->doc_image));
        if($bikeSale->doc_deed) unlink(public_path('uploads/bike-sales/'.$bikeSale->doc_deed));
        $bikeSale->delete();
    ...
    public function approve($id)
    {
        ...
        try {
            ...
            $this->investorTransaction($investorData);
