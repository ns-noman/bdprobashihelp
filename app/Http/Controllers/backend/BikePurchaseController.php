...
class BikePurchaseController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            if(isset($doc_nid)){
                ...
                unlink(public_path('uploads/bike-purchases/'.$bikePurchase->doc_nid));
            }
            if(isset($doc_reg_card)){
                $purchase_data['doc_reg_card'] = $this->documentUpload($doc_reg_card);
                unlink(public_path('uploads/bike-purchases/'.$bikePurchase->doc_reg_card));
            }
            if(isset($doc_image)){
                $purchase_data['doc_image'] = $this->documentUpload($doc_image);
                unlink(public_path('uploads/bike-purchases/'.$bikePurchase->doc_image));
            }
            if(isset($doc_deed)){
                $purchase_data['doc_deed'] = $this->documentUpload($doc_deed);
                unlink(public_path('uploads/bike-purchases/'.$bikePurchase->doc_deed));
            }
    ...
    public function list(Request $request)
    {
        ...
        $query = BikePurchase::join('investors', 'investors.id', '=', 'bike_purchases.investor_id')
                            ...
                            ->join('admins', 'admins.id', '=', 'bike_purchases.created_by_id')
                            ;
        if(!$request->has('order')) $query = $query->orderBy('bike_purchases.id','desc');
    ...
    {
        $bikePurchase = BikePurchase::find($id);
        if($bikePurchase->doc_nid) unlink(public_path('uploads/bike-purchases/'.$bikePurchase->doc_nid));
        if($bikePurchase->doc_reg_card) unlink(public_path('uploads/bike-purchases/'.$bikePurchase->doc_reg_card));
        if($bikePurchase->doc_image) unlink(public_path('uploads/bike-purchases/'.$bikePurchase->doc_image));
        if($bikePurchase->doc_deed) unlink(public_path('uploads/bike-purchases/'.$bikePurchase->doc_deed));
        $bikePurchase->delete();
    ...
    public function approve($id)
    {
        try {
            ...
            $this->investorTransaction($investor_transaction);
